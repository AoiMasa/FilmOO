...
class UserTest {
    ...
    public static before() {
        ...
        const MONGODB_CONNECTION = 'mongodb://localhost:27017/Film00-Test';
    ...
    public async create() {
        ...
        const result = await  (new UserTest.User(this.data)).save();
    ...
    public async getUser(){
        const result = await UserTest.User.findOne({userName : 'brianLove'}).exec();
    ...
    public async updateUser(){
        ...
        const result = await UserTest.User.findOne(
            {userName : 'toto'}).exec();
