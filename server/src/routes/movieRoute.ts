...
export class MovieRoute extends BaseRoute {
    ...
    public getRouter(): express.Router {
        ...
        router.get('/findonebyid/:id', this.getMovieById);
        router.get('/actorStats/:userid', this.getActors);
    ...
    private getActors = (req: Request, res: Response, next: NextFunction) => {

        return this.db.user.findOne({_id: req.params.userid}).exec().then(user => {
            if (user != null) {
                return this.db.movie.aggregate([
                    {$match: {_id: {$in: user.movies}}},
                    {$unwind: '$actors'},
                    {
                        $group: {
                            _id: '$actors',
